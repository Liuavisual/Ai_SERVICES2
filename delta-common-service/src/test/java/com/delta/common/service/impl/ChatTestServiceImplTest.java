...
public class ChatTestServiceImplTest {
    ...
    void testTimeBasedOrderIntent() {
        when(keywordMatcherService.matchFirst(eq("就今天玩儿，给我预约好"), anyList())).thenReturn("就今天");
    ...
    void testScheduleInquiryShouldHandoff() {
        when(keywordMatcherService.matchFirst(eq("勇者者行动有人接吗"), anyList())).thenReturn("有人接吗");
    ...
    void testSpecificCompanionRequest() {
        when(keywordMatcherService.matchFirst(eq("我要找红狼陪我打"), anyList())).thenReturn("我要找");
    ...
    void testNegativeEmotionTriggersHandoff() {
        when(keywordMatcherService.matchFirst(eq("什么破服务，太垃圾了"), anyList())).thenReturn("垃圾");
